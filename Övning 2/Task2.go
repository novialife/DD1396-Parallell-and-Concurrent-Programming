...
func main() {
	...
	wgp.Add(producers) //Adds 4 waitgroups, ie. 4 producers to wait for.
	wgc.Add(consumers) // Add wg for consumers too

	for i := 0; i < producers; i++ {
		go Produce("p"+strconv.Itoa(i), strings/producers, ch, wgp)
	}
	for i := 0; i < consumers; i++ { // Consumes all the channel data after producers is finished.
...
func Produce(id string, n int, ch chan<- string, wgp *sync.WaitGroup) {
	for i := 0; i < n; i++ {
		RandomSleep(100) // Simulate time to produce data.
		ch <- id + ":" + strconv.Itoa(i)
	}
	wgp.Done() // Decrements the waitgroup by 1, meaning 1 producer is finished
}
